CLASS zzzz2ui5_cl_util_db DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zzzz2ui5_cl_util_db IMPLEMENTATION.



  METHOD delete_by_handle.

    DELETE FROM zzzz2ui5_t_91
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF zzzz2ui5_t_91 WITH EMPTY KEY.
  ...
  METHOD save.
    ...
    DATA(ls_db) = VALUE zzzz2ui5_t_91(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
        data    = zzzz2ui5_cl_util=>xml_stringify( data ) ).

    TRY.
        ls_db-id = lt_db[ 1 ]-id.
      CATCH cx_root.
        ls_db-id = zzzz2ui5_cl_util=>uuid_get_c32( ).
    ENDTRY.

    MODIFY zzzz2ui5_t_91 FROM @ls_db.
    ASSERT sy-subrc = 0.
