CLASS z2ui7_cl_ajson_utilities DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      RETURNING
        VALUE(ro_instance) TYPE REF TO z2ui7_cl_ajson_utilities.
    METHODS diff
      IMPORTING
        !iv_json_a            TYPE string OPTIONAL
        !iv_json_b            TYPE string OPTIONAL
        !io_json_a            TYPE REF TO z2ui7_if_ajson OPTIONAL
        !io_json_b            TYPE REF TO z2ui7_if_ajson OPTIONAL
        !iv_keep_empty_arrays TYPE abap_bool DEFAULT abap_false
      EXPORTING
        !eo_insert            TYPE REF TO z2ui7_if_ajson
        !eo_delete            TYPE REF TO z2ui7_if_ajson
        !eo_change            TYPE REF TO z2ui7_if_ajson
      RAISING
    ...
    METHODS merge
      IMPORTING
        ...
        !iv_keep_empty_arrays TYPE abap_bool DEFAULT abap_false
      RETURNING
        VALUE(ro_json)        TYPE REF TO z2ui7_if_ajson
      RAISING
        z2ui7_cx_ajson_error .
    METHODS sort
      IMPORTING
        !iv_json         TYPE string OPTIONAL
        !io_json         TYPE REF TO z2ui7_if_ajson OPTIONAL
      RETURNING
    ...
    METHODS is_equal
      IMPORTING
        !iv_json_a            TYPE string OPTIONAL
        !iv_json_b            TYPE string OPTIONAL
        !ii_json_a            TYPE REF TO z2ui7_if_ajson OPTIONAL
        !ii_json_b            TYPE REF TO z2ui7_if_ajson OPTIONAL
      RETURNING
  ...
  PRIVATE SECTION.
    ...
    DATA mo_json_a TYPE REF TO z2ui7_if_ajson .
    DATA mo_json_b TYPE REF TO z2ui7_if_ajson .
    DATA mo_insert TYPE REF TO z2ui7_if_ajson .
    DATA mo_delete TYPE REF TO z2ui7_if_ajson .
    DATA mo_change TYPE REF TO z2ui7_if_ajson .
...
CLASS z2ui7_cl_ajson_utilities IMPLEMENTATION.
    ...
    DO.
      ...
      IF iv_keep_empty_arrays = abap_false.
        LOOP AT io_json->mt_json_tree INTO ls_json_tree
          WHERE type = z2ui7_if_ajson_types=>node_type-array AND children = 0.
      ...
      LOOP AT io_json->mt_json_tree INTO ls_json_tree
        WHERE type = z2ui7_if_ajson_types=>node_type-object AND children = 0.
  ...
  METHOD diff.
    ...
    mo_insert = z2ui7_cl_ajson=>create_empty( ).
    mo_delete = z2ui7_cl_ajson=>create_empty( ).
    mo_change = z2ui7_cl_ajson=>create_empty( ).
  ...
  METHOD is_equal.
    ...
    DATA li_ins TYPE REF TO z2ui7_if_ajson.
    DATA li_del TYPE REF TO z2ui7_if_ajson.
    DATA li_mod TYPE REF TO z2ui7_if_ajson.
  ...
  METHOD normalize_input.
    ...
    IF boolc( iv_json IS INITIAL ) = boolc( io_json IS INITIAL ).
      z2ui7_cx_ajson_error=>raise( 'Either supply JSON string or instance, but not both' ).
    ENDIF.

    IF iv_json IS NOT INITIAL.
      ro_json = z2ui7_cl_ajson=>parse( iv_json ).
    ELSEIF io_json IS NOT INITIAL.
      ro_json = io_json.
    ELSE.
      z2ui7_cx_ajson_error=>raise( 'Supply either JSON string or instance' ).
    ENDIF.
  ...
  METHOD sort.
    ...
    DATA lo_json TYPE REF TO z2ui7_if_ajson.
