CLASS zzzz2ui5_cl_core_srv_bind DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.

    DATA mo_app    TYPE REF TO zzzz2ui5_cl_core_app.
    DATA mr_attri  TYPE REF TO zzzz2ui5_if_core_types=>ty_s_attri.
    DATA ms_config TYPE zzzz2ui5_if_core_types=>ty_s_bind_config.
    DATA mv_type   TYPE string.

    METHODS constructor
      IMPORTING
        app TYPE REF TO zzzz2ui5_cl_core_app.

    METHODS main_local
      IMPORTING
        val           TYPE data
        config        TYPE zzzz2ui5_if_core_types=>ty_s_bind_config OPTIONAL
      RETURNING
...
CLASS zzzz2ui5_cl_core_srv_bind IMPLEMENTATION.
  METHOD bind_tab_cell.
    ...
    DATA(lt_attri) = zzzz2ui5_cl_util=>rtti_get_t_attri_by_any( ms_config-tab ).
  ...
  METHOD check_raise_existing.
    ...
    IF mr_attri->custom_mapper IS BOUND.
      ...
      DATA(lv_name1) = zzzz2ui5_cl_util=>rtti_get_classname_by_ref( mr_attri->custom_mapper ).
      DATA(lv_name2) = zzzz2ui5_cl_util=>rtti_get_classname_by_ref( ms_config-custom_mapper ).
  ...
  METHOD get_client_name.
    ...
    result = replace( val  = result
                      ...
                      with = ``
                      occ  = 0 ).
    result = COND #( WHEN mv_type = zzzz2ui5_if_core_types=>cs_bind_type-two_way
                     THEN |/{ zzzz2ui5_if_core_types=>cs_ui5-two_way_model }| )
        && |/{ result }|.
  ...
  METHOD main.
    ...
    IF zzzz2ui5_cl_util=>check_bound_a_not_inital( config-tab ).
    ...
    DATA(lo_model) = NEW zzzz2ui5_cl_core_srv_attri( attri = mo_app->mt_attri
                                                  app   = mo_app->mo_app ).
    ...
    IF |/{ zzzz2ui5_if_core_types=>cs_ui5-two_way_model }| = result.
      RAISE EXCEPTION TYPE zzzz2ui5_cx_util_error
        EXPORTING
  ...
  METHOD main_cell.
    ...
    DATA(lo_bind) = NEW zzzz2ui5_cl_core_srv_bind( mo_app ).
  ...
  METHOD main_local.
    TRY.
        ...
        DATA(lo_json) = CAST zzzz2ui5_if_ajson( zzzz2ui5_cl_ajson=>new( ) ).
        lo_json->set( iv_path = `/`
                      iv_val  = val ).
        ...
        ELSE.
          lo_json = lo_json->map( zzzz2ui5_cl_ajson_mapping=>create_upper_case( ) ).
        ENDIF.

        IF config-custom_filter IS BOUND.
          lo_json = lo_json->filter( config-custom_filter ).
        ELSE.
          lo_json = lo_json->filter( zzzz2ui5_cl_ajson_filter_lib=>create_empty_filter( ) ).
        ENDIF.

        DATA(lv_id) = to_upper( zzzz2ui5_cl_util=>uuid_get_c22( ) ).
        INSERT VALUE #( name_client     = |/{ lv_id }|
                        name            = lv_id
                        json_bind_local = lo_json
                        bind_type       = zzzz2ui5_if_core_types=>cs_bind_type-one_time )
  ...
  METHOD update_model_attri.
    ...
    mr_attri->view               = COND #( WHEN ms_config-view IS INITIAL
                                           THEN zzzz2ui5_if_client=>cs_view-main
                                           ELSE ms_config-view ).
