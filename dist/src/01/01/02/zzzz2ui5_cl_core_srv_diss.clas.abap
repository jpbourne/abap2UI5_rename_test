CLASS zzzz2ui5_cl_core_srv_diss DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        attri TYPE REF TO zzzz2ui5_if_core_types=>ty_t_attri
  ...
  PROTECTED SECTION.
    ...
    DATA mt_attri TYPE REF TO zzzz2ui5_if_core_types=>ty_t_attri.
    ...
    METHODS diss_struc
      IMPORTING
        ir_attri      TYPE REF TO zzzz2ui5_if_core_types=>ty_s_attri
      RETURNING
        VALUE(result) TYPE zzzz2ui5_if_core_types=>ty_t_attri.
    ...
    METHODS create_new_entry
      IMPORTING
        !name         TYPE string
      RETURNING
        VALUE(result) TYPE zzzz2ui5_if_core_types=>ty_s_attri.

  PRIVATE SECTION.
ENDCLASS.


CLASS zzzz2ui5_cl_core_srv_diss IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD create_new_entry.
    ...
    result = VALUE zzzz2ui5_if_core_types=>ty_s_attri( ).
    result-name = name.
    DATA(lo_model) = NEW zzzz2ui5_cl_core_srv_attri( attri = mt_attri
                                                  app   = mo_app ).
  ...
  METHOD diss_dref.
    ...
    IF zzzz2ui5_cl_util=>check_unassign_inital( ir_attri->r_ref ).
      RETURN.
    ENDIF.

    DATA(lr_ref) = zzzz2ui5_cl_util=>unassign_data( ir_attri->r_ref ).
    IF lr_ref IS INITIAL.
      RETURN.
    ENDIF.

    DATA(ls_attri2) = VALUE zzzz2ui5_if_core_types=>ty_s_attri( ).
  ...
  METHOD diss_oref.
    ...
    DATA(lr_ref) = zzzz2ui5_cl_util=>unassign_object( ir_attri->r_ref ).
    DATA(lt_attri) = zzzz2ui5_cl_util=>rtti_get_t_attri_by_oref( lr_ref ).
  ...
  METHOD diss_struc.
    ...
    DATA(lt_attri) = zzzz2ui5_cl_util=>rtti_get_t_attri_by_any( lr_ref ).
  ...
  METHOD main_init.
    ...
    IF mt_attri->* IS NOT INITIAL.
      LOOP AT mt_attri->* TRANSPORTING NO FIELDS
           WHERE bind_type <> zzzz2ui5_if_core_types=>cs_bind_type-one_time.
      ENDLOOP.
      IF sy-subrc = 0.
        RETURN.
      ENDIF.
    ...
    DATA(ls_attri) = VALUE zzzz2ui5_if_core_types=>ty_s_attri( r_ref = REF #( mo_app ) ).
  ...
  METHOD main_run.
    ...
    DATA(lt_attri_new) = VALUE zzzz2ui5_if_core_types=>ty_t_attri( ).

    LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri)
         WHERE check_dissolved  = abap_false
               AND bind_type       <> zzzz2ui5_if_core_types=>cs_bind_type-one_time.
