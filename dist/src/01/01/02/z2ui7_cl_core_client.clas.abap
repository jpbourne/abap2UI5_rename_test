CLASS z2ui7_cl_core_client DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES z2ui7_if_client.

    DATA mo_action TYPE REF TO z2ui7_cl_core_action.

    METHODS constructor
      IMPORTING
        action TYPE REF TO z2ui7_cl_core_action.
...
CLASS z2ui7_cl_core_client IMPLEMENTATION.
  ...
  METHOD z2ui7_if_client~follow_up_action.
...
*    mo_action->ms_next-s_set-s_follow_up_action-custom_js = val.
  ...
  METHOD z2ui7_if_client~get_event_arg.
  ...
  METHOD z2ui7_if_client~get_app.

    IF id IS NOT INITIAL.
      DATA(lo_app) = z2ui7_cl_core_app=>db_load( id ).
  ...
  METHOD z2ui7_if_client~message_box_display.

    IF z2ui7_cl_util=>rtti_check_clike( text ) = abap_false.
      DATA(lt_msg) = z2ui7_cl_util=>msg_get( text ).
      IF lines( lt_msg ) = 1.
        DATA(lv_text) = lt_msg[ 1 ]-text.

        DATA(lv_type) = z2ui7_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ...
      ELSEIF lines( lt_msg ) > 1.
        ...
        IF title IS INITIAL.
          lv_title = SWITCH #( lt_msg[ 1 ]-type
                               ...
                               ELSE `Information` ).
        ENDIF.
        lv_type = z2ui7_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ELSE.
        RETURN.
      ENDIF.
    ELSE.
  ...
  METHOD z2ui7_if_client~message_toast_display.
  ...
  METHOD z2ui7_if_client~nav_app_call.
    ...
    IF app->id_app IS INITIAL.
      app->id_app = z2ui7_cl_util=>uuid_get_c32( ).
    ENDIF.
    result = app->id_app.
  ENDMETHOD.

  METHOD z2ui7_if_client~nav_app_leave.

    IF app IS NOT SUPPLIED.
      app = z2ui7_if_client~get_app( z2ui7_if_client~get( )-s_draft-id_prev_app_stack ).
    ENDIF.
  ...
  METHOD z2ui7_if_client~nest2_view_destroy.

    mo_action->ms_next-s_set-s_view_nest2-check_update_model = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~nest2_view_display.
  ...
  METHOD z2ui7_if_client~nest2_view_model_update.

    mo_action->ms_next-s_set-s_view_nest2-check_update_model = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~nest_view_destroy.

    mo_action->ms_next-s_set-s_view_nest-check_update_model = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~nest_view_display.
  ...
  METHOD z2ui7_if_client~nest_view_model_update.

    mo_action->ms_next-s_set-s_view_nest-check_update_model = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~popover_destroy.

    mo_action->ms_next-s_set-s_popover-check_destroy = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~popover_display.
  ...
  METHOD z2ui7_if_client~view_model_update.

    mo_action->ms_next-s_set-s_view-check_update_model = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~_bind.

    DATA(lo_bind) = NEW z2ui7_cl_core_srv_bind( mo_action->mo_app ).
    result = lo_bind->main( val    = z2ui7_cl_util=>conv_get_as_data_ref( val )
                            type   = z2ui7_if_core_types=>cs_bind_type-one_way
                            config = VALUE #( path_only            = path
                                              custom_filter        = custom_filter
                                              custom_mapper        = custom_mapper
                                              tab                  = z2ui7_cl_util=>conv_get_as_data_ref( tab )
                                              tab_index            = tab_index
  ...
  METHOD z2ui7_if_client~_bind_edit.

    DATA(lo_bind) = NEW z2ui7_cl_core_srv_bind( mo_action->mo_app ).
    result = lo_bind->main( val    = z2ui7_cl_util=>conv_get_as_data_ref( val )
                            type   = z2ui7_if_core_types=>cs_bind_type-two_way
                            config = VALUE #( path_only            = path
                                              custom_filter        = custom_filter
  ...
  METHOD z2ui7_if_client~_bind_local.
  ...
  METHOD z2ui7_if_client~_event.

    DATA(lo_ui5) = NEW z2ui7_cl_core_srv_event( ).
    ...
    IF r_data IS NOT INITIAL.
      ...
      mo_action->ms_next-r_data = z2ui7_cl_util=>conv_copy_ref_data( r_data ).
    ENDIF.

  ENDMETHOD.

  METHOD z2ui7_if_client~_event_client.
  ...
  METHOD z2ui7_if_client~set_nav_back.

    mo_action->ms_next-s_set-set_nav_back = val.

  ENDMETHOD.

  METHOD z2ui7_if_client~set_push_state.
  ...
  METHOD z2ui7_if_client~set_app_state_active.

    mo_action->ms_next-s_set-set_app_state_active = val.

  ENDMETHOD.

  METHOD z2ui7_if_client~set_session_stateful.

    DATA(lv_check_sticky) = CAST z2ui7_if_app( mo_action->mo_app->mo_app )->check_sticky.
    ...
    IF val = abap_true.
      ...
      CAST z2ui7_if_app( mo_action->mo_app->mo_app )->check_sticky = abap_true.
    ELSE.
      mo_action->ms_next-s_set-s_stateful-active = 0.
      CAST z2ui7_if_app( mo_action->mo_app->mo_app )->check_sticky = abap_false.
    ENDIF.
    mo_action->ms_next-s_set-s_stateful-switched = abap_true.

  ENDMETHOD.

  METHOD z2ui7_if_client~check_app_prev_stack.

    DATA(ls_get) = z2ui7_if_client~get( ).
    result = xsdbool( ls_get-s_draft-id_prev_app_stack IS NOT INITIAL ).

  ENDMETHOD.

  METHOD z2ui7_if_client~check_on_init.

    result = xsdbool( CAST z2ui7_if_app( mo_action->mo_app->mo_app )->check_initialized = abap_false ).

  ENDMETHOD.

  METHOD z2ui7_if_client~check_on_navigated.
  ...
  METHOD z2ui7_if_client~get_app_prev.

    DATA(ls_get) = z2ui7_if_client~get( ).
    result = z2ui7_if_client~get_app( ls_get-s_draft-id_prev_app ).
