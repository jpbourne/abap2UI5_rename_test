CLASS z2ui7_cl_core_srv_draft DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    TYPES ty_s_db TYPE z2ui7_t_01.
    ...
    METHODS create
      IMPORTING
        draft     TYPE z2ui7_if_types=>ty_s_draft
    ...
    METHODS read_info
      IMPORTING
        id            TYPE clike
      RETURNING
        VALUE(result) TYPE z2ui7_if_types=>ty_s_draft.
...
CLASS z2ui7_cl_core_srv_draft IMPLEMENTATION.

  METHOD cleanup.

    DATA(lv_four_hours_ago) = z2ui7_cl_util=>time_substract_seconds( time    = z2ui7_cl_util=>time_get_timestampl( )
                                                                     seconds = 60 * 60 * 4 ).

    DELETE FROM z2ui7_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE ty_s_db( id                = draft-id
                                 ...
                                 timestampl        = z2ui7_cl_util=>time_get_timestampl( )
                                 data              = model_xml ).

    MODIFY z2ui7_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
