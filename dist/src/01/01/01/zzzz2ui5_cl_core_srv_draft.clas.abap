CLASS zzzz2ui5_cl_core_srv_draft DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    TYPES ty_s_db TYPE zzzz2ui5_t_01.
    ...
    METHODS create
      IMPORTING
        draft     TYPE zzzz2ui5_if_types=>ty_s_draft
    ...
    METHODS read_info
      IMPORTING
        id            TYPE clike
      RETURNING
        VALUE(result) TYPE zzzz2ui5_if_types=>ty_s_draft.
...
CLASS zzzz2ui5_cl_core_srv_draft IMPLEMENTATION.

  METHOD cleanup.

    DATA(lv_four_hours_ago) = zzzz2ui5_cl_util=>time_substract_seconds( time    = zzzz2ui5_cl_util=>time_get_timestampl( )
                                                                     seconds = 60 * 60 * 4 ).

    DELETE FROM zzzz2ui5_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE ty_s_db( id                = draft-id
                                 ...
                                 timestampl        = zzzz2ui5_cl_util=>time_get_timestampl( )
                                 data              = model_xml ).

    MODIFY zzzz2ui5_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
  ...
  METHOD read.
    ...
    IF check_load_app = abap_true.
      ...
      SELECT SINGLE * FROM zzzz2ui5_t_01
        WHERE id = @id
  ...
  METHOD count_entries.
    ...
    SELECT COUNT( * ) FROM zzzz2ui5_t_01
      INTO @result.
