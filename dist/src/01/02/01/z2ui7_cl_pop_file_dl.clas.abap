CLASS z2ui7_cl_pop_file_dl DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result)       TYPE REF TO z2ui7_cl_pop_file_dl.
...
CLASS z2ui7_cl_pop_file_dl IMPLEMENTATION.
  METHOD factory.
  ...
  METHOD view_display.
    ...
    DATA(popup) = z2ui7_cl_xml_view=>factory_popup( )->dialog( title      = title
                                                               icon       = icon
                                                               afterclose = client->_event( 'BUTTON_CANCEL' )
              )->content( ).

    IF mv_check_download = abap_true.
      DATA(lv_csv_x) = z2ui7_cl_util=>conv_get_xstring_by_string( mv_value ).
      DATA(lv_base64) = z2ui7_cl_util=>conv_encode_x_base64( lv_csv_x ).
