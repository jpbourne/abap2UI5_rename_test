CLASS zabap2ui5_cl_layo_pop DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    INCLUDE TYPE zabap2ui5_t_11.
    TYPES   selkz TYPE abap_bool.
    TYPES END OF ty_s_layo.
    TYPES ty_t_layo TYPE STANDARD TABLE OF ty_s_layo WITH EMPTY KEY.

    DATA mo_layout           TYPE REF TO zabap2ui5_cl_layo_manager.
    DATA mt_controls         TYPE zabap2ui5_cl_layo_manager=>ty_t_controls.
    ...
    CLASS-METHODS on_event_layout
      IMPORTING
        !client TYPE REF TO zabap2ui5_if_client
        !layout TYPE REF TO zabap2ui5_cl_layo_manager.

    CLASS-METHODS render_layout_function
      IMPORTING
        !xml          TYPE REF TO zabap2ui5_cl_xml_view
        !client       TYPE REF TO zabap2ui5_if_client
        !layout       TYPE REF TO zabap2ui5_cl_layo_manager
      RETURNING
        VALUE(result) TYPE REF TO zabap2ui5_cl_xml_view.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(result) TYPE REF TO zabap2ui5_cl_layo_pop.

  PROTECTED SECTION.
    DATA client  TYPE REF TO zabap2ui5_if_client.
    ...
    METHODS check_width_unit
      IMPORTING
        !width        TYPE zabap2ui5_t_12-width
      RETURNING
        VALUE(result) TYPE zabap2ui5_t_12-width.
...
CLASS zabap2ui5_cl_layo_pop IMPLEMENTATION.
  ...
  METHOD on_init.
    ...
    CASE Control.
      WHEN zabap2ui5_cl_layo_manager=>m_table.
      ...
      WHEN zabap2ui5_cl_layo_manager=>ui_table.
        mt_halign = VALUE #( ( low = 'Begin'     ddtext = 'Locale-specific positioning at the beginning of the line' )
                             ( low = 'Center'    ddtext = 'Centered text alignment'                                  )
                             ( low = 'End'       ddtext = 'Locale-specific positioning at the end of the line'       )
                             ( low = 'Left'      ddtext = 'Hard option for left alignment'                           )
    ...
    mt_controls = zabap2ui5_cl_layo_manager=>get_controls( ).

  ENDMETHOD.

  METHOD render_edit.

    DATA(popup) = zabap2ui5_cl_xml_view=>factory_popup( ).
...
*                                  stretch      = abap_true
    ...
    DATA(lt_comp) = zabap2ui5_cl_util=>rtti_get_t_attri_by_any( mo_layout->ms_layout-t_layout ).
  ...
  METHOD save_layout.
    ...
    DATA line      TYPE zabap2ui5_t_12.
    DATA Positions TYPE STANDARD TABLE OF zabap2ui5_t_12 WITH EMPTY KEY.
    ...
    DATA(Head) = VALUE zabap2ui5_t_11( guid     = mo_layout->ms_layout-s_head-guid
                                        layout   = mv_layout
    ...
    SELECT guid FROM zabap2ui5_t_11
    ...
    IF sy-subrc = 0.
      ...
      IF t_heads IS NOT INITIAL.
        ...
        SELECT mandt, guid, pos_guid FROM zabap2ui5_t_12
        ...
        IF sy-subrc = 0.
          ...
          DELETE zabap2ui5_t_12 FROM TABLE @t_del.
    ...
    MODIFY zabap2ui5_t_11 FROM @Head.
    IF sy-subrc = 0.

      MODIFY zabap2ui5_t_12 FROM TABLE @Positions.
...
*      INTO CORRESPONDING FIELDS OF TABLE @mo_layout->ms_layout-t_layout  ##SUBRC_OK.
    ...
    mo_layout = zabap2ui5_cl_layo_manager=>factory_by_guid( layout_guid = head-guid ).
  ...
  METHOD delete_selected_layout.
    ...
    DELETE FROM zabap2ui5_t_11 WHERE guid = @Head-guid.

    DELETE FROM zabap2ui5_t_12 WHERE guid = @Head-guid.
  ...
  METHOD render_add_subcolumn.
    ...
    DATA(lo_popup) = zabap2ui5_cl_xml_view=>factory_popup( ).
  ...
  METHOD on_event_subcoloumns.
    ...
    CASE client->get( )-event.
      ...
      WHEN `SUBCOLUMN_ADD`.
        INSERT VALUE #( key = zabap2ui5_cl_util=>uuid_get_c32( ) ) INTO TABLE mo_layout->mt_sub_cols.
        client->popup_model_update( ).
