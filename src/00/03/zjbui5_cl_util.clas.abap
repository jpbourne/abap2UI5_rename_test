CLASS zjbui5_cl_util DEFINITION
  PUBLIC
  INHERITING FROM zjbui5_cl_util_abap
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS rtti_get_t_ddic_fixed_values
      ...
      RETURNING
        VALUE(result) TYPE zjbui5_cl_util_abap=>ty_t_fix_val ##NEEDED.
    ...
    CLASS-METHODS filter_get_sql_where
      IMPORTING
        val           TYPE  zjbui5_cl_util=>ty_t_filter_multi
      RETURNING
        VALUE(result) TYPE string.
    ...
    CLASS-METHODS itab_get_by_struc
      IMPORTING
        val           TYPE any
      RETURNING
        VALUE(result) TYPE zjbui5_cl_util=>ty_t_name_value.
...
*                      v = c_trim_lower( lv_value ) ) INTO TABLE rt_params.
  ...
  METHOD xml_srtti_stringify.
    ...
    ELSE.
      ...
      TRY.
          CALL METHOD zjbui5_cl_srt_typedescr=>('CREATE_BY_DATA_OBJECT')
            EXPORTING
              data_object = data
            RECEIVING
              srtti       = srtti.
...
*    result  = VALUE z2ui5_cl_util=>ty_t_name_value( ).
    LOOP AT lt_attri REFERENCE INTO DATA(lr_attri).
      ...
      CASE zjbui5_cl_util=>rtti_get_type_kind( <component> ).
...
*         e_t_where      = lt_where
      ...
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE zjbui5_cx_util_error
          EXPORTING
            val = zjbui5_cl_util=>context_get_sy( ).
      ENDIF.
  ...
  METHOD msg_get_t.
    ...
    result = zjbui5_cl_util_msg=>msg_get( val ).
  ...
  METHOD rtti_get_data_element_text_l.
    ...
    result = zjbui5_cl_util=>rtti_get_data_element_texts( val )-long.
  ...
  METHOD msg_get_by_msg.
    ...
    DATA(ls_msg) = VALUE zjbui5_cl_util=>ty_s_msg(
      id         = id
      no         = no
      v1         = v1
      v2         = v2
