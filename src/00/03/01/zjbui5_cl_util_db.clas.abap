CLASS zjbui5_cl_util_db DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zjbui5_cl_util_db IMPLEMENTATION.



  METHOD delete_by_handle.

    DELETE FROM zjbui5_t_91
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF zjbui5_t_91 WITH EMPTY KEY.
  ...
  METHOD save.
    ...
    DATA(ls_db) = VALUE zjbui5_t_91(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
        data    = zjbui5_cl_util=>xml_stringify( data ) ).

    IF lv_id IS NOT INITIAL.
      ls_db-id = lv_id.
    ELSE.
      ls_db-id = zjbui5_cl_util=>uuid_get_c32( ).
    ENDIF.

    MODIFY zjbui5_t_91 FROM @ls_db.
    ASSERT sy-subrc = 0.
