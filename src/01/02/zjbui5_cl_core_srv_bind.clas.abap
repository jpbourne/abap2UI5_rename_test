...
CLASS zjbui5_cl_core_srv_bind IMPLEMENTATION.
  METHOD bind_tab_cell.
    ...
    DATA(lt_attri) = zjbui5_cl_util=>rtti_get_t_attri_by_any( ms_config-tab ).
  ...
  METHOD check_raise_existing.
    ...
    IF mr_attri->custom_mapper IS BOUND.
      ...
      DATA(lv_name1) = zjbui5_cl_util=>rtti_get_classname_by_ref( mr_attri->custom_mapper ).
      DATA(lv_name2) = zjbui5_cl_util=>rtti_get_classname_by_ref( ms_config-custom_mapper ).
  ...
  METHOD main_local.
    TRY.
        ...
        ELSE.
          lo_json = lo_json->map( zjbui5_cl_ajson_mapping=>create_upper_case( ) ).
        ENDIF.

        IF config-custom_filter IS BOUND.
          lo_json = lo_json->filter( config-custom_filter ).
        ELSE.
          lo_json = lo_json->filter( zjbui5_cl_ajson_filter_lib=>create_empty_filter( ) ).
        ENDIF.

        DATA(lv_id) = to_upper( zjbui5_cl_util=>uuid_get_c22( ) ).
