CLASS zjbui5_cl_core_action DEFINITION
  ...
  PUBLIC SECTION.
    ...
    DATA mo_http_post TYPE REF TO zjbui5_cl_core_http_post.
    ...
    METHODS constructor
      IMPORTING
        val TYPE REF TO zjbui5_cl_core_http_post.
...
CLASS zjbui5_cl_core_action IMPLEMENTATION.
  ...
  METHOD factory_stack_leave.
    ...
    "check for new app?
    TRY.
        DATA(lo_draft) = NEW zjbui5_cl_core_draft_srv( ).
  ...
  METHOD factory_system_startup.
    ...
    result->mo_app->ms_draft-id          = zjbui5_cl_util=>uuid_get_c32( ).
    result->ms_actual-check_on_navigated = abap_true.
    result->mo_app->mo_app               = zjbui5_cl_core_app_startup=>factory( ).
