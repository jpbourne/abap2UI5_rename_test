CLASS zjbui5_cl_core_diss_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PROTECTED SECTION.
    ...
    METHODS diss_oref
      IMPORTING ir_attri      TYPE REF TO zjbui5_if_core_types=>ty_s_attri
      RETURNING VALUE(result) TYPE zjbui5_if_core_types=>ty_t_attri.
...
CLASS zjbui5_cl_core_diss_srv IMPLEMENTATION.
  ...
  METHOD diss_dref.
    ...
    IF zjbui5_cl_util=>check_unassign_inital( ir_attri->r_ref ).
      RETURN.
    ENDIF.

    DATA(lr_ref) = zjbui5_cl_util=>unassign_data( ir_attri->r_ref ).
  ...
  METHOD diss_oref.
    ...
    DATA(lr_ref) = zjbui5_cl_util=>unassign_object( ir_attri->r_ref ).
  ...
  METHOD main_init.
    ...
    IF mt_attri->* IS NOT INITIAL.
      LOOP AT mt_attri->* TRANSPORTING NO FIELDS
        WHERE bind_type <> zjbui5_if_core_types=>cs_bind_type-one_time.
      ENDLOOP.
      IF sy-subrc = 0.
        RETURN.
      ENDIF.
    ...
    DATA(ls_attri) = VALUE zjbui5_if_core_types=>ty_s_attri( r_ref = REF #( mo_app ) ).
  ...
  METHOD main_run.
    ...
    LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri)
         ...
         AND bind_type <> zjbui5_if_core_types=>cs_bind_type-one_time.
