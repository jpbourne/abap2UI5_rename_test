...
CLASS zjbui5_cl_core_app IMPLEMENTATION.
  ...
  METHOD all_xml_stringify.
    ...
    DATA(lo_dissolver) = NEW zjbui5_cl_core_srv_model( attri = mt_attri
                                                     app    = mo_app ).
...
*        endif.
  ...
  METHOD db_load.
    ...
    DATA(lo_model) = NEW zjbui5_cl_core_srv_model( attri = result->mt_attri
                                                  app   = result->mo_app ).
  ...
  METHOD model_json_parse.
    ...
    DATA(lo_json_mapper) = NEW zjbui5_cl_core_srv_model(
      attri = mt_attri
      app   = mo_app ).
