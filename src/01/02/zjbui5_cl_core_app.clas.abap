CLASS zjbui5_cl_core_app DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    DATA ms_draft TYPE zjbui5_if_types=>ty_s_get-s_draft.
    ...
    METHODS model_json_parse
      IMPORTING
        iv_view  TYPE clike
        io_model TYPE REF TO zjbui5_if_ajson.
    ...
    CLASS-METHODS all_xml_parse
      IMPORTING
        !xml          TYPE clike
      RETURNING
        VALUE(result) TYPE REF TO zjbui5_cl_core_app.
...
CLASS zjbui5_cl_core_app IMPLEMENTATION.

  METHOD all_xml_parse.

    zjbui5_cl_util=>xml_parse( EXPORTING xml = xml
  ...
  METHOD all_xml_stringify.
    ...
    TRY.
        ...
        result = zjbui5_cl_util=>xml_stringify( me ).
      ...
      CATCH cx_root INTO DATA(x2).
        TRY.
            ...
            DATA(lo_dissolver) = NEW zjbui5_cl_core_srv_diss( attri = mt_attri
                                                             app   = mo_app ).

            lo_dissolver->main( ).
            lo_dissolver->main( ).
            lo_model = NEW zjbui5_cl_core_srv_attri( attri = mt_attri
                                                    app   = mo_app ).
            lo_model->attri_before_save( ).
  ...
  METHOD db_load.
    ...
    DATA(lo_db) = NEW zjbui5_cl_core_srv_draft( ).
    DATA(ls_db) = lo_db->read_draft( id ).
    result = all_xml_parse( ls_db-data ).

    DATA(lo_model) = NEW zjbui5_cl_core_srv_attri( attri = result->mt_attri
                                                  app   = result->mo_app ).
  ...
  METHOD db_save.
    ...
    IF mo_app IS BOUND.
      CAST zjbui5_if_app( mo_app )->id_draft = ms_draft-id.
      CAST zjbui5_if_app( mo_app )->check_initialized = abap_true.
    ENDIF.
