CLASS zjbui5_cl_core_srv_model DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        attri TYPE REF TO zjbui5_if_core_types=>ty_t_attri
    ...
    METHODS main_attri_search
      ...
      RETURNING
        VALUE(result) TYPE REF TO zjbui5_if_core_types=>ty_s_attri.
  ...
  PROTECTED SECTION.
    ...
    DATA mt_attri TYPE REF TO zjbui5_if_core_types=>ty_t_attri.
    ...
    METHODS diss_struc
      IMPORTING
        ir_attri      TYPE REF TO zjbui5_if_core_types=>ty_s_attri
      RETURNING
        VALUE(result) TYPE zjbui5_if_core_types=>ty_t_attri.
    ...
    METHODS attri_create_new
      IMPORTING
        !name         TYPE string
      RETURNING
        VALUE(result) TYPE zjbui5_if_core_types=>ty_s_attri.

  PRIVATE SECTION.
ENDCLASS.


CLASS zjbui5_cl_core_srv_model IMPLEMENTATION.
    ...
    LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri)
      ...
      TRY.
          ...
          IF lr_attri->srtti_data IS NOT INITIAL.
            ASSIGN lr_ref->* TO FIELD-SYMBOL(<val>).
            <val> = zjbui5_cl_util=>xml_srtti_parse( lr_attri->srtti_data ).
    ...
    LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri)
      ...
      CASE lo_descr->type_kind.
        ...
        WHEN cl_abap_datadescr=>typekind_table.
          ...
          LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri_child)
            ...
            IF sy-subrc <> 0.
              CONTINUE.
            ENDIF.
            lr_attri->srtti_data = zjbui5_cl_util=>xml_srtti_stringify( <val_ref> ).
            CLEAR <val_ref>.
            CLEAR <val1>.
            CLEAR <ref>.
            EXIT.
        ...
        WHEN cl_abap_datadescr=>typekind_struct1 OR cl_abap_datadescr=>typekind_struct2.
          lr_attri->srtti_data = zjbui5_cl_util=>xml_srtti_stringify( <val1> ).
  ...
  METHOD attri_create_new.
    ...
    result = VALUE zjbui5_if_core_types=>ty_s_attri( ).
    result-name = name.
  ...
  METHOD diss_dref.
    ...
    IF zjbui5_cl_util=>check_unassign_inital( lr_ref_tmp ).
      RETURN.
    ENDIF.

    DATA(lr_ref) = zjbui5_cl_util=>unassign_data( lr_ref_tmp ).
    IF lr_ref IS INITIAL.
      RETURN.
    ENDIF.

    DATA(ls_attri2) = VALUE zjbui5_if_core_types=>ty_s_attri( ).
  ...
  METHOD diss_oref.
    ...
    DATA(lr_ref) = zjbui5_cl_util=>unassign_object( lr_ref_tmp ).
    DATA(lt_attri) = zjbui5_cl_util=>rtti_get_t_attri_by_oref( lr_ref ).
  ...
  METHOD diss_struc.
    ...
    IF lr_ref IS BOUND.
      DATA(lt_attri) = zjbui5_cl_util=>rtti_get_t_attri_by_any( lr_ref ).
  ...
  METHOD dissolve_run.
    ...
    IF mt_attri->* IS INITIAL.
      DATA(ls_attri) = VALUE zjbui5_if_core_types=>ty_s_attri( ).
      DATA(lt_init) = diss_oref( REF #( ls_attri ) ).
      INSERT LINES OF lt_init INTO TABLE mt_attri->*.
    ENDIF.

    DATA(lt_attri_new) = VALUE zjbui5_if_core_types=>ty_t_attri( ).
