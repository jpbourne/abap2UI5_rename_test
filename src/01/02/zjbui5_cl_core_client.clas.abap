...
CLASS zjbui5_cl_core_client IMPLEMENTATION.
  ...
  METHOD zjbui5_if_client~message_box_display.
    ...
    IF zjbui5_cl_util=>rtti_check_clike( text ) = abap_false.
      DATA(lt_msg) = zjbui5_cl_util=>msg_get_t( text ).
      IF lines( lt_msg ) = 1.
        DATA(lv_text) = lt_msg[ 1 ]-text.

        DATA(lv_type) = zjbui5_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ...
      ELSEIF lines( lt_msg ) > 1.
        ...
        IF title IS INITIAL.
          lv_title = SWITCH #( lt_msg[ 1 ]-type
                               ...
                               ELSE `Information` ).
        ENDIF.
        lv_type = zjbui5_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ELSE.
        RETURN.
      ENDIF.
    ELSE.
  ...
  METHOD zjbui5_if_client~view_display.
    ...
    IF zjbui5_cl_util=>rtti_check_clike( val ).
  ...
  METHOD zjbui5_if_client~_bind.
    ...
    DATA(lo_bind) = NEW zjbui5_cl_core_srv_bind( mo_action->mo_app ).
    result = lo_bind->main( val    = zjbui5_cl_util=>conv_get_as_data_ref( val )
  ...
  METHOD zjbui5_if_client~_event.
    ...
    DATA(lo_ui5) = NEW zjbui5_cl_core_srv_event( ).
    ...
    IF r_data IS NOT INITIAL.
      ...
      mo_action->ms_next-r_data = zjbui5_cl_util=>conv_copy_ref_data( r_data ).
    ENDIF.
  ...
  METHOD zjbui5_if_client~set_nav_back.

    mo_action->ms_next-s_set-set_nav_back = val.

  ENDMETHOD.

  METHOD zjbui5_if_client~set_push_state.
  ...
  METHOD zjbui5_if_client~set_app_state_active.
  ...
  METHOD zjbui5_if_client~check_app_prev_stack.

    DATA(ls_get) = zjbui5_if_client~get( ).
    result = xsdbool( ls_get-s_draft-id_prev_app_stack IS NOT INITIAL ).

  ENDMETHOD.

  METHOD zjbui5_if_client~check_on_init.

    result = xsdbool( CAST zjbui5_if_app( mo_action->mo_app->mo_app )->check_initialized = abap_false ).

  ENDMETHOD.

  METHOD zjbui5_if_client~check_on_navigated.
  ...
  METHOD zjbui5_if_client~get_app_prev.

    DATA(ls_get) = zjbui5_if_client~get( ).
    result = zjbui5_if_client~get_app( ls_get-s_draft-id_prev_app ).
