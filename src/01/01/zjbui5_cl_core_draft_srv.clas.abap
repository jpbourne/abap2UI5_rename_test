CLASS zjbui5_cl_core_draft_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zjbui5_cl_core_draft_srv IMPLEMENTATION.


  METHOD cleanup.

    DATA(lv_four_hours_ago) = zjbui5_cl_util=>time_substract_seconds(
        time    = zjbui5_cl_util=>time_get_timestampl( )
        seconds = 60 * 60 * 4 ).
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE zjbui5_if_core_types=>ty_s_db(
        id                = draft-id
