  ...
  PUBLIC SECTION.
    ...
    METHODS read_draft
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE zjbui5_if_core_types=>ty_s_db.
...
CLASS zjbui5_cl_core_srv_draft IMPLEMENTATION.
  METHOD cleanup.
    ...
    DATA(lv_four_hours_ago) = zjbui5_cl_util=>time_substract_seconds( time    = zjbui5_cl_util=>time_get_timestampl( )
                                                                     seconds = 60 * 60 * 4 ).

    DELETE FROM zjbui5_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE zjbui5_if_core_types=>ty_s_db( id                = draft-id
                                                      id_prev           = draft-id_prev
                                                      id_prev_app       = draft-id_prev_app
                                                      id_prev_app_stack = draft-id_prev_app_stack
                                                      uname             = zjbui5_cl_util=>context_get_user_tech( )
