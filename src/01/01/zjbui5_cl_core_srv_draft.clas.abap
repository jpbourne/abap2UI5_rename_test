CLASS zjbui5_cl_core_srv_draft DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    TYPES ty_s_db TYPE zjbui5_t_01.
    ...
    METHODS create
      IMPORTING
        draft     TYPE zjbui5_if_types=>ty_s_draft
    ...
    METHODS read_info
      IMPORTING
        id            TYPE clike
      RETURNING
        VALUE(result) TYPE zjbui5_if_types=>ty_s_draft.
...
CLASS zjbui5_cl_core_srv_draft IMPLEMENTATION.

  METHOD cleanup.

    DATA(ls_config) = VALUE zjbui5_if_types=>ty_s_http_config_post( ).
    zjbui5_cl_exit=>get_instance( )->set_config_http_post( CHANGING cs_config = ls_config ).

    DATA(lv_n_hours_ago) = zjbui5_cl_util=>time_substract_seconds( time    = zjbui5_cl_util=>time_get_timestampl( )
                                                                  seconds = 60 * 60 * ls_config-draft_exp_time_in_hours ).

    DELETE FROM zjbui5_t_01 WHERE timestampl < @lv_n_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE ty_s_db( id                = draft-id
                                 ...
                                 timestampl        = zjbui5_cl_util=>time_get_timestampl( )
                                 data              = model_xml ).

    MODIFY zjbui5_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
  ...
  METHOD read.
    ...
    IF check_load_app = abap_true.
      ...
      SELECT SINGLE * FROM zjbui5_t_01
        WHERE id = @id
