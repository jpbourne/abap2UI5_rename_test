CLASS zjbui5_cl_app_startup DEFINITION
  PUBLIC FINAL
  CREATE PROTECTED.

  PUBLIC SECTION.
    INTERFACES zjbui5_if_app.
    ...
    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO zjbui5_cl_app_startup.
  ...
  PROTECTED SECTION.
    DATA mt_classes TYPE zjbui5_cl_util=>ty_t_classes.

  PRIVATE SECTION.
ENDCLASS.


CLASS zjbui5_cl_app_startup IMPLEMENTATION.
  ...
  METHOD on_event_check.
    ...
    TRY.
        DATA li_app_test TYPE REF TO zjbui5_if_app.
        ms_home-classname = zjbui5_cl_util=>c_trim_upper( ms_home-classname ).
        ...
        ms_home-url               = zjbui5_cl_core_srv_util=>app_get_url( client    = client
  ...
  METHOD view_display_start.
    ...
    DATA(page) = zjbui5_cl_xml_view=>factory( )->shell( )->page(
    ...
    IF zjbui5_cl_util=>rtti_check_class_exists( 'z2ui5_cl_app_icf_config' ).
    ...
    DATA(lv_url_samples2) = zjbui5_cl_core_srv_util=>app_get_url( client    = client
                                                                 classname = 'z2ui5_cl_demo_app_000' ).

    simple_form->toolbar( )->title( `What's next?` ).

    IF zjbui5_cl_util=>rtti_check_class_exists( `z2ui5_cl_demo_app_000` ).
  ...
  METHOD zjbui5_if_app~main.
    ...
    IF client->get( )-check_on_navigated = abap_true.
      TRY.
          DATA(lo_f4) = CAST zjbui5_cl_pop_to_select( client->get_app_prev( ) ).
  ...
  METHOD view_display_popup.
    ...
    DATA(page2) = zjbui5_cl_xml_view=>factory_popup(
    ...
    simple_form2->checkbox( enabled  = abap_false
                            selected = zjbui5_cl_util=>context_check_abap_cloud( ) ).

    simple_form2->label( `Userexit` ).
    simple_form2->text( zjbui5_cl_exit=>get_user_exit_class( ) ).

    DATA(lv_count) = CONV string( NEW zjbui5_cl_core_srv_draft( )->count_entries( ) ).
    simple_form2->toolbar( )->title( `abap2UI5` ).
    simple_form2->label( `Version ` ).
    simple_form2->text( zjbui5_if_app=>version ).
  ...
  METHOD z2ui5_on_event.
    ...
    DATA li_app TYPE REF TO zjbui5_if_app.

    CASE client->get( )-event.

      WHEN `SET_CONFIG`.
        DATA lo_app TYPE REF TO zjbui5_if_app.
      ...
      WHEN 'VALUE_HELP'.
        TRY.
            mt_classes = zjbui5_cl_util=>rtti_get_classes_impl_intf( zjbui5_cl_util=>rtti_get_intfname_by_ref( li_app ) ).
          CATCH cx_root.
            client->message_box_display( `Unfortunately the value help is not available on your ABAP release!` ).
            RETURN.
        ENDTRY.
        client->nav_app_call( zjbui5_cl_pop_to_select=>factory( mt_classes ) ).
    ENDCASE.
  ...
  METHOD z2ui5_on_init.
    ...
    ms_home-class_editable = abap_true.
    ms_home-btn_icon       = `sap-icon://validate`.
    ms_home-classname      = zjbui5_cl_util=>rtti_get_classname_by_ref( NEW zjbui5_cl_app_hello_world( ) ).
