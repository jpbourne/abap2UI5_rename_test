CLASS zjbui5_cl_pop_to_select DEFINITION
  PUBLIC FINAL
  CREATE PROTECTED.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result)    TYPE REF TO zjbui5_cl_pop_to_select.
...
CLASS zjbui5_cl_pop_to_select IMPLEMENTATION.
  ...
  METHOD factory.
    ...
    r_result->mr_tab            = zjbui5_cl_util=>conv_copy_ref_data( i_tab ).
  ...
  METHOD display.
    ...
    DATA(tab) = popup->table_select_dialog(
        items            = |\{path:'|
                          ...
                          && zjbui5_cl_util=>boolean_abap_2_json( me->descending )
                          && | \} \}|
    ...
    DATA(lt_comp) = zjbui5_cl_util=>rtti_get_t_attri_by_any( <tab_out> ).
    ...
    LOOP AT lt_comp INTO ls_comp.
      DATA(text) = COND #(
                     LET data_element_name = substring_after(
                                                 ...
                                                 sub = '\TYPE=' )
                         medium_label = zjbui5_cl_util=>rtti_get_data_element_texts( data_element_name )-medium IN
